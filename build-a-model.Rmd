...
To do that such function should compare the output of our model with the measured data we are using for calibration.  

**You will need a time series of measurements for optimizing your model. You can find the example here: [Ultuna_LTBF_data.csv](./Ultuna_LTBF_data.csv). Donwload it and put it in the same folder than your R script. **.  
In case you did not set up your home directory to this folder, remember that the following commands will not work (the `read.csv` command will not find the file and complain).  


We will then encapsulate in this function the function we wrote above and the solver. The "cost" is a metric we want to minimize, usually some metric of model fitness.
